...
import { designSystemModel } from "./model";
import { validateDesignSystemDocument } from "./validation";
...
describe("DesignSystemDocument", () => {
  ...
  it("uses the same pure validation flow for normalized documents and assessment", async () => {
    const document = structuredClone(catalogFixtures[0].document);
    document.identity.tags = ["  Premium ", "premium", "Product"];

    const validation = await validateDesignSystemDocument(document);

    expect(validation).toMatchObject({
      kind: "assessed",
      document: { identity: { tags: ["Premium", "Product"] } },
      assessment: { diagnostics: expect.any(Array), artifacts: expect.any(Object) },
    });
  });

  it("enforces structural numeric bounds and raw collection limits", async () => {
    const valid = structuredClone(catalogFixtures[0].document);
    const negativeSize = structuredClone(valid);
    negativeSize.foundations.typography.roles.body!.size = -1;
    const excessiveWeight = structuredClone(valid);
    excessiveWeight.foundations.typography.roles.body!.weight = 1001;
    const tooManyRawTags = structuredClone(valid);
    tooManyRawTags.identity.tags = ["Data", "data", "A", "B", "C", "D"];

    expect("issues" in designSystemModel.schema.decoder["~standard"].validate(valid)).toBe(false);
    for (const document of [negativeSize, excessiveWeight, tooManyRawTags])
      expect(designSystemModel.schema.decoder["~standard"].validate(document)).toMatchObject({
  ...
  it("keeps every seeded catalog fixture publishable", () => {
    expect(new Set(catalogFixtures.map(({ fixtureId }) => fixtureId)).size).toBe(
      catalogFixtures.length,
    );
    expect(new Set(catalogFixtures.map(({ slug }) => slug)).size).toBe(catalogFixtures.length);
    expect(catalogFixtures.every(({ slug }) => /-[a-f0-9]{12}$/.test(slug))).toBe(true);
    ...
    const tactile = catalogFixtures.find(({ fixtureId }) => fixtureId === "tactile")?.renderer;
  ...
  it("blocks severe contrast failures and warns on marginal AA contrast", () => {
    ...
    const transparentAction = structuredClone(
      catalogFixtures.find(({ fixtureId }) => fixtureId === "tactile")!.document,
    );
  ...
  it("validates normalized tags before returning the canonical document", async () => {
    const document = structuredClone(catalogFixtures[0].document);
